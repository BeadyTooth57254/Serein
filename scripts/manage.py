    ...
    while True:
        try:
            ...
            choice=choose('Serein · 安装与维护',[
                ('0','设置前端用户名／密码'),('1','部署 Serein（本地源码）'),('2','重启服务'),('3','向量重建与清理'),('4','启动／停止／状态／日志'),('5','访问入口：本机／局域网／公网'),('6','更换 Gateway Key'),('7','网页旧库目录只读授权'),('8','旧边转换补救（早期版本未转换成功）'),('9','旧备份清理'),('10','历史数据补漏（梦境／窗影／日记／暗房）'),('11','拉取上游代码并重建'),('q','退出')])
            if choice=='q':return
