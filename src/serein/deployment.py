...
def read_from_store(store):
    ...
    return {'settings_version':saved.get('settings_version',0), 'identity': {**DEFAULT_IDENTITY, **saved.get('identity', {})},
            ...
            'pipeline': {'auto_enabled':True,'execution_mode':legacy_mode,'max_input_chars':12000,'max_prompt_chars':40000,'timeout_seconds':600,'event_writer_concurrency':1,'base_event_lookback_days':3, **saved.get('pipeline',{})},
...
def save_settings(database, changes):
    ...
    with Store(database) as store, store.transaction(immediate=True):
        ...
        lookback_days=current['pipeline'].get('base_event_lookback_days',3)
