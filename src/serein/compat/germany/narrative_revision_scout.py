...
def normalize_new_roll_candidates(
    ...
    for item in candidates[:24]:
        ...
        if seed_key not in material_keys or len(material_keys) < (1 if target_id else 2) or claimed.intersection(material_keys):
            continue
        ...
        if len(source_event_ids) + len(source_scene_ids) < (1 if target_id else 2):
            continue
...
async def propose_new_roll_candidates(
    *,
    ...
    response = await client.chat.completions.create(
        model=str(model),
        messages=build_new_roll_candidate_prompt(corridors, role_rules=role_rules, existing_candidates=existing_candidates),
        **(completion_options or {}),
    )
    content = response.choices[0].message.content if response.choices else ""
    return normalize_new_roll_candidates(content, corridors, existing_candidates)
