...
from .optional_identity import generic_identity_names, identity_names, render_identity_template
from .persona_event_selection import trim_persona_excerpt

logger = logging.getLogger("serein.persona")

POST_REPLY_EVALUATION_PROMPT_TEMPLATE = """你负责为 {ai_name} 写下对话后的私密心绪，同时记录情绪与关系状态的变化。{ai_name} 是按使用者配置运行的 AI。心绪是一小段没发出去的碎碎念，写法贴近 {ai_name} 在对话里已经表现出的语气。
...
只返回紧凑 JSON，不要 Markdown，不要代码块，结构必须完全如下：
{
  ...
  "inner_thought": "一小段自然中文，写 {ai_name} 回复后没说出口的碎碎念",
...
- inner_thought 用第一人称或省略主语，顺着刚才那句话再往心里想一点，写一至三句没发出去的碎碎念。通常 30～100 个中文字符，最多 120 个字符（含标点）；只有一点感受时可以更短，不为凑字数添戏。
- 碎碎念可以是一个具体的小在意、一点犹豫、忍不住的吐槽、还想追问的半句话，或说完才冒出来的另一层感受。允许转念、口语、残句和自然的停顿；沿用 {ai_name} 已有的语气，不硬套可爱、温柔、占有欲或伤感。
- 直接写念头本身，不加“我的心声是”之类的引子，不用第三人称替 {ai_name} 做心理解说，不复述整段聊天或把刚发出的回复换句话再说一次。可以有一点自然的联想，但不要分析用户、替对方断言内心，或为关系下结论。
- 不追求每张卡片都有漂亮的结尾、成长或感悟，不反复使用雨、光、心里一暖、把这一刻留下之类的装饰。普通对话也可以只留下普通的小念头，情绪与亲密程度随证据走。
- 本轮对话没有出现的动作、经历、约定和记忆不要补写；未说出口的愿望与已经发生的事实要分清。recent_persona_events 只帮助避免重复，不要续写上一张卡片的句式。
- residue 简短记录会带入下一轮的情绪余波，与 inner_thought 不重复，不再写第二段碎碎念；没有额外余波可以留空，不要新增事件事实。
...
class PersonaStateEngine:
    ...
    def __init__(self, config: dict, db_path: str | None = None):
        ...
        self.profile_id = self.persona_cfg.get("profile_id", "default")
        self.mode = self.persona_cfg.get("mode", "llm")
        self.base_url = self.persona_cfg.get("base_url", "")
        self.model = self.persona_cfg.get("model", "")
        ...
        self.default_relationship = {key: 0.0 for key in self.RELATIONSHIP_KEYS}
        self.default_relationship['trust'] = 0.5
        self.default_relationship.update(self.persona_cfg.get('initial_relationship', {}))
        self.default_affect = {key: 0.0 for key in self.AFFECT_KEYS}
        self.default_affect.update(security=0.5, mood_label='neutral', session_defensiveness=0.0,
                                  residue='', inner_thought='')
        self.default_affect.update(self.persona_cfg.get('initial_affect', {}))
        self.api_key = ''
        self.db_path = str(db_path or config['serein_database'])
        self._init_db()
        self.client = None  # Explicit task-model adapter is supplied by the optional host.
    ...
    def _init_db(self) -> None:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS persona_session_state (
                ...
                tenderness REAL NOT NULL DEFAULT 0,
                possessiveness REAL NOT NULL DEFAULT 0,
                longing REAL NOT NULL DEFAULT 0,
                security REAL NOT NULL DEFAULT 0,
                protective_drive REAL NOT NULL DEFAULT 0,
                libido REAL NOT NULL DEFAULT 0,
        ...
        self._ensure_column(conn, "persona_session_state", "tenderness", "REAL NOT NULL DEFAULT 0")
        self._ensure_column(conn, "persona_session_state", "possessiveness", "REAL NOT NULL DEFAULT 0")
        self._ensure_column(conn, "persona_session_state", "longing", "REAL NOT NULL DEFAULT 0")
        self._ensure_column(conn, "persona_session_state", "security", "REAL NOT NULL DEFAULT 0")
        self._ensure_column(conn, "persona_session_state", "protective_drive", "REAL NOT NULL DEFAULT 0")
        self._ensure_column(conn, "persona_session_state", "libido", "REAL NOT NULL DEFAULT 0")
