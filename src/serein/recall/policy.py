...
class RecallPolicy:
    ...
    def from_config(cls, raw):
        unknown = raw.keys() - {"direct_threshold", "max_cards", "candidate_limit", "domains", "routing_file", "germany_policy_file", "passages_enabled", "passage_min_chars"}
        if unknown:
        ...
        if not 0 <= policy.direct_threshold <= 1 or not 1 <= policy.max_cards <= 100 or not 1 <= policy.candidate_limit <= 100:
