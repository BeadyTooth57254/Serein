...
import { chronologicalSources } from "./narrativeAppearance.js";
...
function withMaterialIds(roll) {
  ...
  return {
    ...roll,
    sources: chronologicalSources(roll?.sources),
    sourceCount: roll?.sources?.length || 0,
    materialIds: {
      ...inferred,
      ...(roll?.materialIds || {}),
...
function projectLiveRoll(item, index, fallback) {
  ...
  const sources = chronologicalSources(projectSources(item, fallback));
  const paragraphs = splitParagraphs(item.body);
  const isUnwritten = item.publication_status === "collecting" && !String(item.body || "").trim();
  return {
    ...fallback,
    ...
    isUnwritten,
    tone: isUnwritten ? "white" : fallback?.tone || visualTones[index % visualTones.length],
    ...
    paragraphs: isUnwritten ? [] : paragraphs.length ? paragraphs : fallback?.paragraphs || [],
    body: String(item.body || ""),
    sources,
    materialIds: {
      event_ids: (item.linked_event_ids || []).map(String),
    ...
    sourceKind: "serein-narrative-live-readonly",
  };
}
...
export async function loadNarrativeRolls() {
  try {
    ...
    return payload.items
      .filter((item) => item.publication_status === "collecting" || String(item.body || "").trim())
      .map((item, index) => (
