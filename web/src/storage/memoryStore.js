import { initializePersonal } from "./personalStore.js";
...
function mergeLiveMemoryProjection(snapshotScenes, liveProjection, { includeSnapshotOnly = true } = {}) {
  ...
  liveProjection.scenes.forEach((liveScene) => {
    ...
    const sources = fallback?.sources?.length
      ? fallback.sources
      : [{ id: `manual_source:${sourceId}`, kind: "Serein v1 原文", title: `只读来源 · ${sourceId}` }];
    sceneIdBySourceId.set(sourceId, sceneId);
    mergedById.set(sceneId, {
      ...fallback,
      id: sceneId,
      date: String(liveScene.date || fallback?.date || "").slice(0, 10),
      ...
      annotations: Array.isArray(liveScene.annotations) ? liveScene.annotations : [],
      sources,
      ...
      storageStatus: String(liveScene.storage_status || ""),
      storageType: String(liveScene.type || ""),
      storageActive: liveScene.active,
      sceneStatus: String(liveScene.scene_status || ""),
      statusConsistent: liveScene.status_consistent !== false,
      bucketDomain: String(liveScene.bucket_domain || fallback?.bucketDomain || ""),
      selfAnchor: liveScene.self_anchor === true,
      sourceKind: "serein-live-readonly",
...
export function readMemoryScenes() {
  try {
    ...
    return saved
      .filter((scene) => scene?.sourceKind === "serein-live-readonly")
...
async function loadMemorySnapshotOnce() {
  try { await initializePersonal(); } catch { return null; } // Leave the old browser records intact until import succeeds.
  try {
  ...
  try {
    ...
    const scenes = projectedScenes
      .filter((scene) => (
        scene.sourceKind === "serein-live-readonly" || !tombstonedSceneIds.has(scene.id)
      ))
      .map((scene) => {
        const storedScene = savedById.get(scene.id);
        if (scene.sourceKind === "serein-live-readonly") {
