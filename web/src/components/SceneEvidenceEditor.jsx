...
export function SceneEvidenceEditor({ sceneId, sceneTitle }) {
  ...
  const loadSourceMessages = async ({ append = false, query = activeQuery, contextMessageId = 0 } = {}) => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.message || payload.error || "归档原文读取失败");
      ...
      setMessage(error.message || "归档原文读取失败");
    }
  };
  ...
  const bindSelected = async () => {
    ...
    if (!window.confirm(`把选中的 ${selectedIds.size} 条 归档原文绑定到「${sceneTitle}」？\n\n原文快照会绑定到当前记忆，不修改 Scene 正文和向量。`)) return;
    setState("saving");
    setMessage("");
    try {
        ...
        <div className="scene-evidence__list">
          {evidence.map((item) => (
            <article key={item.id || evidenceKey(item)}>
              <header>
                <div>
                  <span>{item.source_system === 'operit' ? 'Operit 记忆' : identityName(item.role)}</span>
                ...
                {item.source_system === "assistant_bridge" && item.message_id ? (
          ...
          {sourceMessages.length ? (
            <div className="scene-evidence-picker__list">
              {sourceMessages.map((item) => {
                ...
                const boundEvidence = evidence.find((evidenceItem) => evidenceKey(evidenceItem) === `assistant_bridge:${item.session_id}:${id}`);
                  ...
                  >
                    <label>
                      ...
                      <span className="scene-evidence-picker__copy">
                        <span>
                          <strong>{identityName(item.role)}</strong>
                          <time>{formatEvidenceTime(item.created_at)}</time>
                          <small>#{id}</small>
