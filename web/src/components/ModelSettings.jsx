...
export function ModelSettings({page,summaryRequest=0,onOpenPipeline,onOpenCatalog,onOpenAssignments,
  ...
  return <>
    ...
    <details className="settings-disclosure" ref={summaryConfig}><summary>自动摘要配置</summary><p>有时间按 20 分钟沉默切块；无时间按 20 轮完整问答切块。每块再受字符量限制，保留完整问答和原始编号。时间切块只控制输入，不直接决定 Event 边界。</p>
      ...
      {Object.entries({max_input_chars:['每批原话字符上限',2000,100000],max_prompt_chars:['完整提示词字符上限',8000,4000000],timeout_seconds:['模型读取超时（秒）',30,1800],event_writer_concurrency:['Event Writer 首轮并发数',1,8],base_event_lookback_days:['归线与旧 Event 回看天数',1,365]}).map(([key,[label,min,max]])=>
        <label className="settings-field" key={key}><span>{label}</span><input type="number" min={min} max={max} value={config.pipeline[key] ?? (key==='base_event_lookback_days'?3:'')} onChange={event=>setConfig(current=>({...current,pipeline:{...current.pipeline,[key]:Number(event.target.value)}}))}/></label>)}
      ...
      <small>默认回看 3 天（72 小时）：归线读取这段时间内实际归入过原话的 Track，不依赖聊天窗口；旧 Event 按整条续写／合并链的首次创建时间筛选，后继记录不重置计时。范围外的卡片和记忆仍保留，新经历可另建 Event。天数修改对新批次生效，已冻结任务保持原材料。</small>
