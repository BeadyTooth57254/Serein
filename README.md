...
进入安装目录运行 `se`，选择 **11 · 拉取上游代码并重建**：用 Git 拉取 `main` 的最新提交，准备完成后停止本实例、按选择备份、更新源码并重新构建启动。无需发布 Release 或发行包；已成功安装同一提交时不重复构建。数据、配置、Key 和用户自建文件保留，Git 拉取失败不会停服。菜单 1 只部署本地已有源码。旧下载包脚本需一次性接入，详见[自动拉取上游更新](docs/interactive-install.md#自动拉取上游更新)。
