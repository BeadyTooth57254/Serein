...
打标时，Event 和 Scene 会在同一次模型请求里获得主域建议与实体候选。实体只收明确命名的人、地点、作品、项目等；每个名字都要能在当前记忆的绑定原话或正文中逐字核对出处，没有可靠实体时允许为空。实体用于寻找候选记忆，不会自动建立记忆关系，也不凭名字直接注入上下文。旧库迁移只给活动 Scene 重新打标；归档记忆和日记不走这一步。细节见 [实体提取规则](docs/entity-tagging.md)。

## 怎么召回

使用 Serein 的聊天网关并开启自动记忆后，每一轮会根据**当前用户消息**寻找可能有帮助的旧记忆。

![自动召回过程：当前消息、判断检索范围、查找 Scene 与 Event、比较相关性、检查当前状态与冷却，最后提供记忆卡片和按需读取的 Arc 目录。](docs/images/recall-flow.svg)

1. **确定查什么。** 结合当前消息判断是否需要召回，并利用实体、主题或 Arc 线索缩小范围。
2. **一起找 Scene 和 Event。** 检索整条正文，长记忆也可通过正文片段命中；命中多个片段不会把同一条记忆变成多张卡。
3. **再比较相关性。** 重排模型比较候选与当前问题。开启联想后，最多再给一条关系相连的 Scene 参与比较，关系本身不会直接获得注入资格。
4. **只带入本轮适合的内容。** 当前默认自动策略最多选两条；还要检查可见性、版本和冷却。没有可靠结果时，不塞入记忆。

向量接近只是候选线索，并不等于应该提起。重排失败时也不会直接拿向量结果代替。召回观察里可以区分找到候选、选中记忆和实际交付；**“搜到了”与“已经交给聊天模型”不是同一步。**
...
![自动摘要流程：已入库原话持续归线，延迟判断入选与续接，Writer 按绑定原文写作，程序检查后直接保存到 Serein；原话和前版关系保留供核对。](docs/images/automatic-events.svg)
...
在 **设置 → 功能** 开启自动摘要，再在配置页选择三阶段使用的模型和 API／Agent 执行方式。使用网关聊天或导入记录，让原话先进入实例；只连接 MCP 不会自动带入所有聊天历史。
...
进入发行目录运行 `se`，选择 **11 · 检查上游更新并安装**：自动从 GitHub 下载最新已发布安装包，校验 SHA-256，再停止本实例、按选择备份、更新源码并重新构建启动。默认先备份；数据、配置、Key 和用户自建文件保留。已是最新版本时不重复构建，下载或校验失败时当前服务继续运行。菜单 1 只负责部署本地已有源码。详见[自动拉取上游更新](docs/interactive-install.md#自动拉取上游更新)。
...
| 保存与管理记忆 | `write_scene`、`edit_scene`、`set_memory_state` | 可写实例；分别新建 Scene、局部修改 Scene、管理状态与自动浮现资格；收藏修改另需开启收藏工具。自动 Event 正文由摘要流水线维护 |
| 记忆候选 | `propose_memory`、`list_candidates`、`review_memory` | 可写实例；提出、接受 Scene 候选，未接受的候选不作为正式记忆召回；历史 Event / Narrative 候选可查看或忽略，不在此接受 |
| 日记与暗室 | `write_diary`、`comment_diary`、`delete_diary` | 可写实例；写条目、评论和软删除，保留作者与暗室解锁约束 |
| 重试索引更新 | `index_sync` | 可写实例；处理记忆已保存但索引尚未同步的任务 |
| 自动摘要 Agent | `pipeline_next`、`pipeline_submit` | 可写实例；领取冻结的阶段任务、提交结果，具体执行方式见 [扩展说明](docs/extensions.md) |

`write_scene` 新建 Scene，直接填 `title`、`content`、`cues`（1–8 条，每条最多 80 字符）、`date`（YYYY-MM-DD），以及用于重试去重的 `operation_id`，不再嵌套 draft。ID 自动生成，不传 `kind`、原文证据或原话 ID。`edit_scene` 编辑已有 Scene，填写 `scene_id`、当前 `expected_revision`、`operation_id` 和要改的字段；省略的字段、已有证据与其他元数据保留。新建时可选 `favorite`；已有记忆的收藏仍用 `set_memory_state`。升级后请刷新客户端工具列表，旧 `save_memory` 已移除，配置白名单中的旧名称自动对应新建与编辑两项能力。叙事卷由主模型通过 `narrative_volume` 的读取、预览、保存流程书写，Event 由原话整理流水线维护。
